...
class FieldInput(object):
    def __init__(self,name,field,vocab=None,max_size=None, min_freq=1, specials=['<pad>'],
                 vectors=None, unk_init=None, vectors_cache=None, **fieldargs):
...
#FieldInput.__new__.__defaults__ = (None,) * 5
...
"""
def visualize(y, x,display=True,save_path=None):
...
def get_model_summary(model,input_tensors,item_length=26,tensorboard_log=None):
    ...
    if not (isinstance(input_tensors,list) or isinstance(input_tensors,tuple)):
...
def one_hot(input,num_classes):
    ...
    output = torch.zeros(out_size).type(input.type())
    return output.scatter(len(input_size) - 1,input,1)

def load_image(path,grayscale=False,target_size=None,to_tensor=True,mean=None,std=None,interpolation = Image.BILINEAR):
